...
--SELECT *
--FROM sys.fn_xe_file_target_read_file('C:\MSSQL_PerformanceAnalyzer\MSSQL_PerformanceAnalyzer\*.xel', NULL, NULL, NULL) x
--OUTER APPLY (SELECT CAST(x.event_data AS xml) xmlToAnalyze) x1
--OUTER APPLY (SELECT 
--				T.c.value('@name', 'varchar(100)') AS result,
--				T.c.value('.', 'varchar(max)') AS result1
--			FROM x1.xmlToAnalyze.nodes('/event/data') T(c)
--			WHERE T.c.value('@name', 'varchar(100)') IN ('duration', 'logical_reads', 'writes','statement')) x2